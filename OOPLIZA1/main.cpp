...
// «адание 1. ¬ариант 2. –адкевич ≈лизавета
...
public:
    fuzzy_number(double e1 = 1.3, double x = 2.1, double e2 = 2.9):x_(x), e1_(e1), e2_(e2)
    { // конструктор
...
int main()
{
    // демонстраци€ математических опреаций
    std::cout << "Math operations test" << std::endl;
    fuzzy_number A(1,2,3), B(4,5,6);
    A+B;
    A-B;
    A*B;
    A/B;
    ...
    // демонстраци€ работы конструкторов
    std::cout << "\nConstructors test" << std::endl;
    fuzzy_number C1;
    fuzzy_number A1(3,2,1);
    fuzzy_number B1 = A1;
    fuzzy_number m1(4,5,6);
    ...
    // демонстраци€ присваивани€
    std::cout << "\nAssignments test" << std::endl;
    fuzzy_number A2,B2(7,8,9);
    A2 = B2;
    fuzzy_number C2;
    A2 = std::move(C2);
}
